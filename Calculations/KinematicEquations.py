...
def forward_kinematics(theta1, theta2, previous_y):
    ...
    Lx = -(a * math.cos(theta1))
    Ly = (a * math.sin(theta1)) 
    Rx = (a * math.cos(theta2)) + w
    Ry = (a * math.sin(theta2))  
    ...
    if discriminant < 1 and discriminant > -1:
        discriminant = 0
    elif discriminant < 0:
...
def encoderToAngle(encoder: int):
    # Encoder value is 8192 t 2*pi
    return encoder * (math.pi/4096)
    

def angleToEncoder(angle: float):
    return (angle * 4096) / math.pi

# theta1=theta2=0.8991
